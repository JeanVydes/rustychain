...
use rustychain::persistent::pgvector::PgVectorStore;
...
impl PgVectorRetrievalTool {
    ...
    pub async fn from_url(database_url: &str, llm: Arc<LLM>) -> rustychain::Result<Self> {
...
impl FnExecutor<AugmentedArgs, serde_json::Value> for PgVectorAugmentedTool {
    async fn call(&self, args: AugmentedArgs) -> rustychain::Result<serde_json::Value> {
        ...
        let documents = embeddings
            .into_iter()
            .map(|(text, embedding)| rustychain::DocumentInput {
                text,
                embedding,

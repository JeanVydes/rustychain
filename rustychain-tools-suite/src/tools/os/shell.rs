...
use rustychain::llm::function::FnExecutor;
use rustychain::{FnDeclarator, FunctionDeclaration};
...
impl CommandTool {
    ...
    async fn get_input_action(&self, context: InputContext) -> rustychain::Result<InputAction> {
    ...
    async fn execute_command(&self, args: CommandArgs) -> rustychain::Result<CommandResult> {
        use tokio::sync::mpsc;
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| rustychain::Error::Generic("Failed to capture stdin".to_string()))?;

        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| rustychain::Error::Generic("Failed to capture stdout".to_string()))?;

        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| rustychain::Error::Generic("Failed to capture stderr".to_string()))?;
...
impl FnExecutor<SignalArgs, serde_json::Value> for SignalTool {
    async fn call(&self, args: SignalArgs) -> rustychain::Result<serde_json::Value> {
        ...
        #[cfg(not(unix))]
        {
            Err(Box::from(rustychain::Error::Generic(
...
impl FnDeclarator<CommandArgs, serde_json::Value> for CommandTool {
    fn declare(
        &self,
    ) -> rustychain::llm::function::FunctionDeclaration<CommandArgs, serde_json::Value> {
        FunctionDeclaration {
            name: "command_tool",

...
impl CommandTool {
    ...
    async fn execute_command(&self, args: CommandArgs) -> crate::Result<CommandResult> {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| crate::Error::Generic("Failed to capture stdin".to_string()))?;

        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| crate::Error::Generic("Failed to capture stdout".to_string()))?;

        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| crate::Error::Generic("Failed to capture stderr".to_string()))?;
...
impl FnExecutor<SignalArgs, serde_json::Value> for SignalTool {
    async fn call(&self, args: SignalArgs) -> crate::Result<serde_json::Value> {
        ...
        #[cfg(not(unix))]
        {
            Err(Box::from(crate::Error::Generic(

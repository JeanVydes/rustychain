...
impl<const N: usize> FnExecutor<KvArgs, serde_json::Value> for KvLruTool<N> {
    async fn call(&self, args: KvArgs) -> crate::Result<serde_json::Value> {
        ...
        match args.action.to_lowercase().as_str() {
            "set" => {
                let key = args
                    .key
                    .ok_or_else(|| crate::Error::Internal("Key required".into()))?;
                let value = args
                    .value
                    .ok_or_else(|| crate::Error::Internal("Value required".into()))?;
